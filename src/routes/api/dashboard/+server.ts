...
export const POST = async ({ request }) => {
	...
	if (!type || !grade)
		return json({ error: 'Cannot determine subject type/grade' }, { status: 400 });
	...
	if (!subjectIds.length)
		return json({ error: 'No subjects found for this type/grade.' }, { status: 400 });
	...
	const { error } = await supabase.from('preferential_round').insert(inserts);
	if (error) return json({ error: error.message }, { status: 500 });
